...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  /**
   * Associates the given value with the given key. If an entry with
   * the key was already in the map, then only returns
   * the existing value, with no changes to the map. Otherwise, a new
   * entry is added and null is returned.
   *
   * @param key   key with which the specified value is to be associated
   * @param value value to be associated with the specified key
  ...
  public V putIfAbsent(K key, V value) {
    ...
    Position<Entry<K, V>> current = treeSearch(tree.root(), key);
    // If key not found, insert it here
    if (isExternal(current)) {
      tree.set(current, new MapEntry<>(key, value));
      tree.addLeft(current, null);  // Make it internal by adding children
      tree.addRight(current, null);
      return null;
    }
