...
public abstract class AbstractTree<E> implements Tree<E> {
  ...
  /**
   * Returns an iterable collection of positions of the tree in depth-first pre-order.
   * @return iterable collection of the tree's positions in depth-first pre-order
   */
  public Iterable<Position<E>> preorderIterative() {
    List<Position<E>> result = new ArrayList<>();

    if (!isEmpty()) {
      Stack<Position<E>> stack = new LinkedStack<>();
      stack.push(root());
      ...
      while (!stack.isEmpty()) {
        ...
        // Collect children first to push in reverse order
        List<Position<E>> kids = new ArrayList<>();
        for (Position<E> child : children(current)) {
          kids.add(child);
        }

        // Push right child first, so left comes out first
  ...
  /**
   * Returns an iterable collection of positions of the tree in depth-first post-order.
   * @return iterable collection of the tree's positions in depth-first post-order
   */
  public Iterable<Position<E>> postorderIterative() {
    List<Position<E>> result = new ArrayList<>();

    if (!isEmpty()) {
      Stack<Position<E>> stack1 = new LinkedStack<>();
      Stack<Position<E>> stack2 = new LinkedStack<>();

      stack1.push(root());

      while (!stack1.isEmpty()) {
        Position<E> current = stack1.pop();
        stack2.push(current);
      ...
      // Final postorder result comes by popping from stack2
